...
* *Support for on-premises to on-premises VMware VMFS datastores for FC*: This release includes support for VMs mounted on VMware vSphere virtual machine file system (VMFS) datastores for FC protected to on-premises storage. Previously, the service provided a _technology preview_ supporting VMFS datastores for FC and iSCSI.  
+
Here are some additional considerations regarding both FC and iSCSI protocols: 
+
...
* *VM details available for failover, failback, and test failover*: When a failure occurs and you are performing a failover, performaing a failback, or testing the failover, you can now see details of the VMs and identify which VMs did not restart. 
+
