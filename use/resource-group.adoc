...
TIP: Each resource group can include one or more VMs. The VMs will power on based on the sequence in which you include them in the replication plan.
...
. Optionally, on the left side of the Add resource groups page, filter the VMs by their datastore. You can also search for specific VMs by name. 
