...
. Optionally, on the left side of the Applications page, filter the VMs by their datastore. 
...
. *Virtual networks*: In the Failover mappings tab, select the down arrow next to *Virtual networks*. Select the source virtual LAN and target segment. 
...
+
*** *DHCP*: Select this setting if you want if you would like your VMs to obtain network configuration information from a DHCP server. If you choose this option, you provide just the credentials for the VM. 
