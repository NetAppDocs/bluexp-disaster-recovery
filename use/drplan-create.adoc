...
+
Under the Virtual machines details, you can optionally resize the VM CPU and RAM parameters. Also, you can select IP address, Subnet Mask, DNS, and Gateway if you want to configure the network information about a VM. VM guest OS credentials should be supplied to either the global level or at each VM level.
//+
