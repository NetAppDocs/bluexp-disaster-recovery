...
+
*IP address, Subnet Mask, DNS, Gateway*: Also, you can select IP address, Subnet Mask, DNS, and Gateway if you want to configure the network information about a VM. VM guest OS credentials should be supplied to either the global level or at each VM level.
+
...
+
*SnapMirror relationships*: If a volume has a SnapMirror relationship established, you can select the corresponding source and target datastores. If you select a volume that does not have a SnapMirror relationship, you can create one now by selecting the working environment and its peer SVM. 
...
NOTE: In this preview, configure the frequency outside of the BlueXP disaster recovery service. 
